...
- **Group by dependency** (`settings.groupByDependency`): when enabled together with a **GitHub token**, after the pulls list DOM is ready (`load` and **`turbo:render`** with a short delay), the extension calls the **GraphQL API** for open PRs: **`viewer.login`**, **refs** (`headRefName` / `baseRefName`), **`reviewDecision`**, and **`latestReviews`** (author `login`/`url`, `state`). It builds a **dependency tree** (`tree.js`), assigns sort indices, draws a **bright solid-color vertical rail** with **depth numbers** (1 = chain root, 2 = direct child, etc.) in a fixed 18px left gutter (`graph-svg.js` + `content-graph.css`), **appends reviewer lines** (approved and/or changes requested — both groups are shown when a PR has reviews of each kind) and **reorders** rows. Row content is **not** indented; only the overlay gutter shows chain membership. Numbers are small plain text (no box); the line breaks above and below each number. **Hover the gutter** for stack/branch context via native `title` / `aria-label` (e.g. `Stack of 6 · position 3 · root #788`, `3 branches · 6 PRs total · root #788`, `Branch 2 of 3 · 2 PRs in branch · root #788`). Injected review markup is tagged with **`data-gce-review`**; graph rows use **`data-gce-graph`** and **`gce-dep-chain`**. Up to **100** open PRs and **100** latest reviews per PR are fetched per request (GitHub API limits).
- **Dual pulls-list DOM** (`pulls-list-dom.js`): supports **legacy** (`.js-navigation-container`, `#issue_*`) and **new ListView** (`ul[data-listview-component="items-list"]`, title link `data-testid="listitem-title-link"`). PR numbers, status/reviewer injection, and Jira badge anchors use the adapter so reordering and datasets work on both layouts.
...
- **Jira ticket badges** (Jira Cloud credentials + **GitHub token**): branch names from GraphQL are matched with a configurable regex; statuses are fetched via the **background** service worker. On **legacy** layout, badges append inside **`span.v-align-middle`**; on **ListView**, they append to the description div (`PullsListItem-module__description`) after existing metadata (tagged **`data-gce-jira`**).
...
| `tree.js` | PR dependency tree from base/head refs (`buildTree`, `countSubtree`, `buildGraphHoverLabel`, `computeGraphMeta`, `getBaseBranchLineColor`) |
| `pulls-list-dom.js` | Legacy + ListView DOM detection, row/PR/status/Jira container helpers |
| `graph-svg.js` | Per-row SVG data URLs: vertical rail + depth numbers in gutter |
| `content-graph.css` | Gutter overlay positioning, hover pointer-events (no row indent) |
| `github-prs.js` | GraphQL: `viewer` + open PRs with refs, reviews, `reviewDecision` |
| `content.js` | Pulls-list URL `q` merge/strip (sort, draft, author), dependency reorder, graph lines, approval row visibility, reviewer UI, storage listener |
