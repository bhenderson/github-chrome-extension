...
- **Jira ticket badges** (Jira Cloud credentials + **GitHub token**): branch names from GraphQL are matched with a configurable regex; statuses are fetched via the **background** service worker. Badges are appended as the last child inside each row’s **`span.v-align-middle`** (tagged **`data-gce-jira`**).
